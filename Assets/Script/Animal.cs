...
public abstract class Animal : MonoBehaviour
{
    ...
    public string Names     //Property
    {
        ...
        private set { names = string.IsNullOrEmpty(value) ? "Unknown Animal" : value; } //Assigns a value to the name variable & Everything we classified as attributes gotta do the same.
    }
    ...
    {
        Debug.Log($"{Names} -> Hunger: {Hunger} | Happiness: {Happiness} | Prefered Food: {PreferredFood}");
    }
    ...
    public virtual void Feed(int foodAmount)
    {
        ...
        Debug.Log($"{Names} was fed {foodAmount} of generic food. Current Happiness: {Happiness}");
    }
    ...
    public virtual void Feed(FoodType foodType, int foodAmount)
    {
        if (foodType == PreferredFood)
        {
            AdjustHunger(-foodAmount);
            AdjustHappiness(+15);
            Debug.Log($"{Names} was fed {foodAmount} units of preferred food : {PreferredFood}, Happiness increased 15 units, Current Happiness {Happiness}");
        }

        else if (foodType == FoodType.RottenFood)
        {
            AdjustHappiness(-20);
            Debug.Log($"{Names} was fed with rotten food : {FoodType.RottenFood}. Unhappy! Happiness decreased 20 units, Current Happiness {Happiness}");
        }
